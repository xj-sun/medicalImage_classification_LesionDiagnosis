...
def main_worker(gpu, ngpus_per_node, args):
    ...
    vardir = os.path.join('test_data')
    if not os.path.isdir(vardir):
        os.makedirs(vardir)
    testdir = os.path.join(vardir, image_name)
    if not os.path.isdir(testdir):
        os.makedirs(testdir)    

    output_file = os.path.join(testdir, image_name)
    os.system('cp \"{}\" \"{}\"'.format(args.data, output_file))
    ...
    val_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(vardir, transforms.Compose([
